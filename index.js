...
const { ChannelType, Client, GatewayIntentBits, AttachmentBuilder } = require('discord.js');
...
const POINT_RULES = Object.freeze({
  actividad: { author: 5, mention: 5, armadaBonus: 2 },
  ayuda: { author: 4, mention: 4, armadaBonus: 2 },
  raid: { author: 10, mention: 10, armadaBonus: 2 },
  sorteo: { author: 4, mention: 0, armadaBonus: 0 }
});

// Keywords que REQUIEREN al menos una mencion valida
const REQUIRES_MENTION = new Set(['actividad', 'raid', 'sorteo']);
...
function loadConfig() {
  // Fix automatico: quita "O" al inicio del ROLE_ID_GUERRERO_DEL_MES si existe
  const guerreroRaw = process.env.ROLE_ID_GUERRERO_DEL_MES?.trim() || '';
  const guerreroClean = guerreroRaw.replace(/^O/, '');

  if (guerreroRaw !== guerreroClean) {
    console.log('[Config] Se detecto y corrigio "O" al inicio de ROLE_ID_GUERRERO_DEL_MES.');
  }
  ...
  return {
    ...
    monthlyRoles: {
      ...
      guerrero: guerreroClean || requiredEnv('ROLE_ID_GUERRERO_DEL_MES')
    }
  };
}
...
function processUserActivityStreak(data, userId) {
  ...
  // Solo permite una actualizacion de racha por dia
...
async function sendStreakBonusAnnouncement(guild, userId, streak, bonusGranted) {
  ...
  const announcement =
    `🔥 <@${userId}> alcanzo una racha de ${streak} dias consecutivos ayudando al gremio!\n` +
...
async function handleSetPointsCommand(message, parts) {
  ...
  await sendMessage(message, `${getDisplayName(targetUser)} ahora tiene ${parsedPoints} puntos.`);
}
...
// ===================== EXPORT BACKUP (ADMIN) =====================
async function handleExportBackupCommand(message) {
  if (!isAdminById(message.author.id)) {
    await sendMessage(message, 'No tienes permiso para usar !export-backup.');
    return;
  }

  try {
    await message.reply('⏳ Generando backup completo de data.json...');

    // Leer archivo actual de disco (datos REALES de Railway)
    const rawContent = fs.readFileSync(DATA_FILE_PATH, 'utf8');
    ...
    // Enviar por DM al admin
    await message.author.send({
      content: `✅ Backup completo generado el ${new Date().toISOString()}\n` +
               `📊 Total usuarios: ${Object.keys(store.users).length}\n` +
               `📝 Total actividades: ${Object.keys(store.activity_logs).length}\n` +
               `📅 Snapshots mensuales: ${store.monthly_snapshots.length}\n\n` +
               `**IMPORTANTE**: Descarga este archivo y guardalo en lugar seguro.`,
      files: [attachment]
    });

    await message.reply('✅ Backup enviado a tu mensaje directo. Revisa tus DMs.');
    console.log(`[EXPORT] Backup generado y enviado a ${message.author.tag}`);

  } catch (error) {
    console.error('[EXPORT] Error al generar backup:', error);
    await sendMessage(message, `❌ Error al generar backup: ${error.message}`);
  }
}
...
async function handleCommand(message) {
  ...
  if (command === 'export-backup') {
    await handleExportBackupCommand(message);
    return true;
  }

  await sendMessage(message, 'Comando no reconocido. Usa !ranking, !ranking total, !puntos, !reset-mensual o !export-backup.');
  return true;
}
...
async function handleActivityMessage(message) {
  ...
  if (keywordInfo.totalMatches === 0) {
    await sendMessage(message, 'Registro invalido: falta una keyword principal valida (#actividad, #ayuda, #raid o #sorteo).');
    return;
  }
  ...
  // #raid y #sorteo requieren al menos una mencion valida
  if (REQUIRES_MENTION.has(keywordInfo.detectedType) && validMentions.length === 0) {
    await sendMessage(message, `Registro invalido: #${keywordInfo.detectedType} requiere al menos una mencion valida.`);
    return;
  }
...
client.once('ready', () => {
  ...
  console.log('Keywords activas: #actividad (5pts), #ayuda (4pts), #raid (10pts), #sorteo (4pts)');
  console.log('Comando !export-backup disponible para admins');
});
